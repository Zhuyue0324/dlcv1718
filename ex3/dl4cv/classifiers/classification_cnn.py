...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.conv = nn.Conv2d(channels, num_filters, kernel_size, stride=stride_conv, padding=int(kernel_size/2))
        self.conv.weight.data.mul_(weight_scale)
        height1=int((height+2*int(kernel_size/2)-kernel_size)/stride_conv)+1
        width1=int((width+2*int(kernel_size/2)-kernel_size)/stride_conv)+1

        self.maxpool2d=nn.MaxPool2d(kernel_size=pool, stride=stride_pool)

        height2=int((height1-pool)/stride_pool)+1
        width2=int((width1-pool)/stride_pool)+1
        
        self.fc1 = nn.Linear(num_filters * height2 * width2, hidden_dim)
    ...
    def forward(self, x):
        ...
        # layers.                                                              #
        ########################################################################
        x = self.maxpool2d(F.relu(self.conv(x)))
